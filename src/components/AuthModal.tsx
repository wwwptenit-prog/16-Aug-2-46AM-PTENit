...
import { X, LogIn, UserPlus, Lock, Mail, Phone, Briefcase, Wrench, Zap, Eye, EyeOff, ShieldCheck, HelpCircle, ArrowRight } from 'lucide-react';
...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const { login, signup, demoLogin } = useData();
  ...
  const handleDemoClick = (role: 'admin' | 'customer' | 'instructor') => {
    demoLogin(role);
    setErrorMsg('');
    onSuccess();
    onClose();
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-md w-full p-6 sm:p-7 relative shadow-2xl space-y-5 text-slate-900 dark:text-white my-auto max-h-[92vh] overflow-y-auto font-bengali">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">
            {mode === 'signup' ? 'নতুন অ্যাকাউন্ট খুলুন — সহজ ও নিরাপদ' : 'আপনার অ্যাকাউন্টে সাইন ইন করুন'}
          </p>
        ...
        <div className="pt-3 border-t border-slate-200 dark:border-slate-800 space-y-2">
          <p className="text-[11px] font-bold text-slate-500 dark:text-slate-400 flex items-center gap-1">
            <HelpCircle className="w-3.5 h-3.5 text-[#1DB954]" />
            <span>সরাসরি ডেমো লগইন পরীক্ষা করুন:</span>
          </p>
          ...
          <div className="grid grid-cols-3 gap-1.5 text-xs">
            <button
              ...
              className="py-2 px-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-[#1DB954] hover:text-white rounded-xl font-bold border border-slate-200 dark:border-slate-700 transition cursor-pointer flex items-center justify-center gap-1 text-[11px]"
            >
              <Briefcase className="w-3.5 h-3.5" />
              <span>গ্রাহক (Buyer)</span>
            </button>
